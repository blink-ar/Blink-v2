...
/**
 * Component that displays benefits using your preferred Benefit type
 */
    ...
    <div className="bg-white rounded-lg shadow-md p-6 mb-4 border border-gray-200">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        <div>
          ...
          {benefit.locations.map((location, index) => (
            <p key={index} className="text-sm text-gray-600">
              {location.formattedAddress || `${location.lat}, ${location.lng}`}
            </p>
          ))}
        </div>
        <div>
      ...
      <div className="border-t pt-3 text-xs text-gray-400">
        ...
        {benefit.sourceMeta && (
          <>
            <p>Fuente: {benefit.sourceMeta.type}</p>
            <p>
              Agregado:{" "}
              {new Date(benefit.sourceMeta.addedAt).toLocaleDateString()}
            </p>
          </>
        )}
      </div>
