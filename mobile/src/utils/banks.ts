...
const asBankText = (value: unknown): string => {
  ...
  if (value && typeof value === 'object') {
    ...
    for (const k of ['bank', 'name', 'label', 'code', 'value', 'id']) {
...
export const toBankDescriptor = (bankValue: unknown): BankDescriptor => {
  ...
  if (!sanitized) return { token: 'bank', code: 'BANK', label: 'BANCO' };
  ...
  const token = words[0];
  const codeSource = words.length > 1 ? words.map((w) => w[0]).join('') : words[0];
  const code = codeSource.slice(0, 4).toUpperCase();

  return { token, code, label: sanitized.toUpperCase().slice(0, 18) };
};

export const compareBankDescriptors = (a: BankDescriptor, b: BankDescriptor) => {
  const knownOrder = new Map(KNOWN_BANKS.map((bank, i) => [bank.token, i]));
...
export const buildBankOptions = (...groups: readonly unknown[][]): BankDescriptor[] => {
  ...
  groups.forEach((group) => {
    group.forEach((bankName) => {
      ...
      if (!optionMap.has(descriptor.token)) optionMap.set(descriptor.token, descriptor);
    });
  });
